...
class TransactionController extends Controller
{
   ...
   public function getMonthlyRecap($month)
   {
      ...
      $transactions = DB::table('transaction_details')
         ...
         ->select(
            'transaction_details.particulars',
            'transaction_details.uacs_code AS code',
            'transaction_details.amount AS debit', // Assuming 'amount' is the debit/credit value
            DB::raw('0 AS credit') // Modify this if you have a separate credit column
         )
         ->get();

      if ($transactions->isEmpty()) {
         return response()->json(['message' => 'No transactions found for this month'], 200);
      }
