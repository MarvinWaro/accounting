<x-app-layout>
    ...
    <style>
        ...
        #search-table thead th {
            padding: 20px 24px;
            /* Adjust these values for top/bottom and left/right padding */
        }
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="mx-5 my-5">
                    <a href="{{ route('transaction.create') }}" type="button"
                        class="my-5 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    ...
                    <div class="table-wrapper overflow-x-auto">
                        <table id="search-table">
                            <thead>
                                <tr>
                                    <th
                                        class="bg-gray-500 text-gray-100 dark:bg-gray-900 dark:text-gray-100 px-10 py-4">
                            ...
                            <tbody>
                                @foreach ($transactions as $transaction)
                                    <tr class="hover:bg-gray-200 dark:hover:bg-gray-700">
                                        <td>
                                            <button id="dropdownButton_{{ $transaction->id }}"
                                                data-dropdown-toggle="dropdown_{{ $transaction->id }}"
                                            ...
                                            <div id="dropdown_{{ $transaction->id }}"
                                                class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                                                    aria-labelledby="dropdownButton_{{ $transaction->id }}">
                                                    <li>
                                                    ...
                                                    <li>
                                                        <form
                                                            ...
                                                            id="delete-form-transaction-{{ $transaction->id }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button id="destroy-btn-transaction-{{ $transaction->id }}"
                                                                type="button"
                                                                class="delete-button w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white flex items-center focus:outline-none">
                                                                <i class="fa-solid fa-trash me-2 text-red-500"></i><span
                                                                    class="text-red-500">Delete</span>
                                                            </button>
                                                        </form>
                                                        <script>
                                                                        ...
                                                                        Swal.fire({
                                                                            ...
                                                                            if (result.isConfirmed) {
                                                                                ...
                                                                                document.getElementById(
                                                                                    `delete-form-transaction-${transactionId}`).submit();
                                                                            }
                                                                        });
                                                                    });
                                                                });
                                        ...
                                        <td>
                                            <ul class="list-disc list-inside">
                                                @foreach ($transaction->details as $detail)
                                                    <li>{{ substr($detail->uacs_code, 0, 1) . '-' . substr($detail->uacs_code, 1, 2) . '-' . substr($detail->uacs_code, 3, 2) . '-' . substr($detail->uacs_code, 5, 3) . '-' . substr($detail->uacs_code, 8, 2) }}
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                ...
                var particulars = $('td .particulars', table.row(dataIndex).node()).data('fulltext')
                    .toLowerCase();
